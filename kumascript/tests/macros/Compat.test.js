...
describeMacro("Compat", function () {
  ...
  // Macro can insert section header automatically, if the third optional
  // argument is "true"
  itMacro(
    ...
    function (macro) {
      return macro.call("api.feature", 1, "true").then(function (result) {
        ...
        assert.equal(header.id, "Browser_compatibility");
        assert.equal(header.textContent, "Browser compatibility");
      });
    }
  );
  itMacro("Does not insert section header by default", function (macro) {
  ...
  itMacro(
    "Adds correct titles for platforms to head of table",
    function (macro) {
      return macro.call("javascript.feature").then(function (result) {
        let dom = JSDOM.fragment(result);
        assert.equal(
  ...
  itMacro(
    "Adds correct classes for browsers to head of table",
    function (macro) {
      return macro.call("javascript.feature").then(function (result) {
        let dom = JSDOM.fragment(result);
        let browserIcons = Array.from(
          dom.querySelectorAll(".bc-browsers span")
        );
        assert.equal(
        ...
        assert.equal(
          browserIcons[3].classList.contains("bc-head-icon-ie"),
          true
        );
        assert.equal(
  ...
  itMacro(
    "Adds correct text label for browsers to head of table",
    function (macro) {
      return macro.call("javascript.feature").then(function (result) {
        let dom = JSDOM.fragment(result);
        let browserIcons = Array.from(
